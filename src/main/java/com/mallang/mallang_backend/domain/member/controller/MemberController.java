...
public class MemberController {
	...
	@ApiResponses(value = {@ApiResponse(responseCode = "200", description = "사용 가능한 이메일입니다."), @ApiResponse(responseCode = "400", description = "파라미터 검증 오류 메시지")})
	@PossibleErrors({DUPLICATE_FILED, MEMBER_NOT_FOUND})
	@PostMapping("/check-email")
	public ResponseEntity<RsData<Void>> checkEmail(
	...
	@ApiResponses({@ApiResponse(responseCode = "200", description = "닉네임 사용 가능 여부 반환"), @ApiResponse(responseCode = "400", description = "파라미터 검증 오류 메시지")})
	 ...
	 */
	@Operation(summary = "회원 정보 변경", description = "회원의 닉네임 / 이메일을 변경할 수 있습니다.")
	@ApiResponse(responseCode = "200", description = "회원 정보가 성공적으로 수정되었습니다.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ChangeInfoResponse.class)))
	@PossibleErrors({DUPLICATE_FILED, MEMBER_NOT_FOUND})
	@PatchMapping("/me")
