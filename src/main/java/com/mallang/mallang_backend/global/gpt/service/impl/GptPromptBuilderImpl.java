...
public class GptPromptBuilderImpl implements GptPromptBuilder {
	...
	public String buildPromptForAnalyzeScript(String script) {
		return String.format("""
        당신은 영어 스크립트를 분석해주는 도우미입니다. 주어진 스크립트를 분석하여 각 문장의 번역과 핵심 단어 정보를 JSON 배열 형식으로 응답하세요.

        입력 형식:
        - 입력은 여러 문장 블록으로 구성되며, 각 블록은 `|` 기호로 구분됩니다.
        - 각 블록은 하나 이상의 영어 문장으로 이루어져 있습니다.

        출력 형식:
        - 출력은 JSON 배열 형태로 작성합니다.
        - 각 블록은 하나의 JSON 객체 배열로 나타내며, 각 문장은 다음 구조를 갖습니다:
        ...
        조건:
        - 각 문장마다 반드시 최소 1개, 최대 3개의 핵심 단어를 추출합니다.
        - 핵심 단어는 반드시 문장에서 **실제로 사용된 형태** 그대로 추출해야 합니다. (예: ceases, existed, going 등)
        - 어형 변화, 복수형, 시제 변화, 동명사 등을 모두 포함하여 문장 내 단어와 정확히 일치해야 합니다.
        - 번역은 앞뒤 문맥에 맞게 자연스러운 한국어로 작성합니다.
        - JSON은 배열로 시작하고 끝나야 하며, 각 객체는 쉼표로 구분됩니다.
        - 기타 설명은 없이 JSON만 출력하세요.
        - 난이도는 1~5 숫자 중 하나로 지정
        - JSON 구조는 반드시 예시 출력과 동일한 구조로 구성
        - 입력 형식에서 '|'로 구분된 문장 블럭 하나 당 반드시 하나의 JSON 객체로 일대일 대응되도록 만들어져야 함
        ...
        [
			...
			{
				"original": "When I was a front line negotiator, it was actually I'm very different when I just started my role in negotiating.",
				"translate": "제가 전방 협상가였을 때, 사실 제가 막 협상 역할을 시작했을 때와는 매우 달랐습니다.",
				"keyword": [
					{
					   "word": "negotiator",
					   "meaning": "협상가",
	...
	public String buildPromptForAnalyzeScriptJapanese(String script) {
		return String.format("""
			   당신은 일본어 스크립트를 분석해주는 도우미입니다. 주어진 스크립트를 분석하여 각 문장의 번역과 핵심 단어 정보를 JSON 배열 형식으로 응답하세요.
			   
			   입력 형식:
			   입력은 여러 문장 블록으로 구성되며, 각 블록은 | 기호로 구분됩니다.
			   각 블록은 하나 이상의 일본어 문장으로 이루어져 있습니다.
			   ...
			   조건:
			   각 문장마다 반드시 최소 1개, 최대 3개의 핵심 단어를 추출합니다.
			   핵심 단어는 반드시 문장에서 실제로 사용된 형태 그대로 추출해야 합니다. (예: 見ている, 存在していた, 行っている 등)
			   어형 변화, 시제, 수식형, 복수형 등을 포함하여 문장 내 실제 표현과 정확히 일치하는 형태로 추출해야 합니다.
			   번역은 문맥에 맞게 자연스러운 한국어로 작성합니다.
			   JSON은 배열로 시작하고 끝나야 하며, 각 객체는 쉼표로 구분됩니다.
			   기타 설명은 없이 JSON만 출력하세요.
			   난이도는 1~5 숫자 중 하나로 지정합니다.
				- JSON 구조는 반드시 예시 출력과 동일한 구조로 구성
				- 입력 형식에서 '|'로 구분된 문장 블럭 하나 당 반드시 하나의 JSON 객체로 일대일 대응되도록 만들어져야 함
			   ...
			   [
			       {
			           "original": "彼は何をしているの？それは私には関係ない。",
			           "translate": "그는 무엇을 하고 있니? 그것은 나와는 상관없어.",
			           "keyword": [
			               {
			                   "word": "している",
			                   "meaning": "하고 있다",
